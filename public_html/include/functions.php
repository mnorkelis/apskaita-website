...
function inisession($arg) {   //valom sesijos kintamuosius
		...
		$_SESSION['pass_login']="";
		$_SESSION['mail_login']="";
        $_SESSION['likutis_login']="";
		$_SESSION['name_error']="";
      	$_SESSION['pass_error']="";
		$_SESSION['mail_error']="";
        $_SESSION['likutis_error']="";
        }
...
function checkname ($username){   // Vartotojo vardo sintakse
 ...
 function checkdb($username) {  // iesko DB pagal varda, grazina {vardas,slaptazodis,lygis,id} ir nustato name_error
	     ...
	     $uname = $upass = $ulevel = $uid = $umail = $likutis = null;
		 if (!$result || (mysqli_num_rows($result) != 1))   // jei >1 tai DB vardas kartojasi, netikrinu, imu pirma
           ...
           $ulevel=$row["userlevel"]; $uid= $row["userid"]; $umail = $row["email"]; $likutis = $row["likutis"];}
     return array($uname,$upass,$ulevel,$uid,$umail,$likutis);
 }
...
function checklikutis ($likutis){   // Vartotojo likucio sintakse
    if (!$likutis || strlen($likutis = trim($likutis)) == 0)
    {$_SESSION['likutis_error']=
        "<font size=\"2\" color=\"#ff0000\">* Neįvestas banko sąsk. likutis!</font>";
        "";
        return false;}
	elseif (!preg_match("/^([0-9a-zA-Z])*$/", $likutis))  /* Check if username is not alphanumeric */
    {$_SESSION['likutis_error']=
        "<font size=\"2\" color=\"#ff0000\">* Vartotojo vardas gali būti sudarytas<br>
				&nbsp;&nbsp;tik iš raidžių ir skaičių</font>";
        return false;}
